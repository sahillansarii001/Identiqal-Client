...
export default function DashboardOverviewPage() {
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      {/* Welcome Banner */}
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 relative overflow-hidden flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl z-0" />
        <div className="relative z-10 space-y-1">
          <h2 className="text-xl font-extrabold text-slate-100">Welcome back, {user?.name}!</h2>
          <p className="text-xs text-slate-400">Manage your smart cards, captured leads, and brand themes here.</p>
        </div>
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        <div className="bg-slate-900/30 border border-slate-900 rounded-2xl p-6 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="flex items-baseline space-x-2">
              <span className="text-2xl font-black text-slate-100">{totalCards}</span>
        ...
        <div className="bg-slate-900/30 border border-slate-900 rounded-2xl p-6 space-y-4">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-black text-slate-100">Leads Hub</span>
        ...
        <div className="bg-slate-900/30 border border-slate-900 rounded-2xl p-6 space-y-4">
          ...
          <div className="flex items-baseline space-x-2">
            <span className="text-2xl font-black text-slate-100">Analytics</span>
      ...
      <div className="bg-slate-900/10 border border-slate-900 rounded-2xl p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-200">Recent Digital Cards</h3>
          <Link href="/dashboard/cards" className="text-xs text-indigo-400 hover:text-indigo-300 font-semibold flex items-center space-x-1">
            <span>View All</span>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10 border border-dashed border-slate-800 rounded-2xl space-y-3">
            <Compass size={32} className="mx-auto text-slate-600" />
            <p className="text-xs text-slate-400">You haven't built any business cards yet.</p>
          ...
          <div className="space-y-3">
            {cards.slice(0, 3).map((card) => (
              <div
                key={card._id}
                className="p-4 bg-slate-900 border border-slate-850 rounded-2xl flex items-center justify-between hover:border-slate-800 transition-colors"
              >
                <div>
