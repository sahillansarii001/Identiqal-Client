  ...
  const buttonRadiusClass =
    {
      ...
      square: "rounded-none",
      outline:
        "rounded-xl border border-current bg-transparent hover:bg-slate-100/10",
        ...
        >
          ...
          <div className="space-y-1">
            <h3
              ...
              style={{
                fontFamily: font.heading || "inherit",
            ...
            >
              {data.bio ||
                "Provide a short biography detailing your role and networking background."}
            </p>
          </div>
        </div>
      );
          ...
          {links.length === 0 ? (
            ...
            links.map((link, idx) => (
              <a
                ...
                className="group flex items-center justify-between p-4 sm:p-5 rounded-2xl bg-black/2 dark:bg-white/2 hover:bg-black/6 dark:hover:bg-white/6 transition-all duration-300"
                style={{ color: colors.primary || "#000000" }}
              >
                <div
                  className="w-10 h-10 rounded-full bg-white dark:bg-black/20 shadow-sm flex items-center justify-center shrink-0"
                  style={{ color: colors.accent || colors.primary }}
                >
                  <span className="text-lg">🔗</span>
                </div>
                <span className="flex-1 px-4 font-bold text-sm sm:text-base tracking-wide">
                  {link.label}
                </span>
    ...
    case "testimonials":
      return (
        ...
        >
          <div
            className="text-2xl"
            style={{ color: colors.accent || "#0d6efd" }}
          >
            “
          </div>
          ...
          <div
            className="flex items-center space-x-3 border-t pt-3"
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] opacity-60">
                {data.authorTitle || "Title / Organization"}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {images.map((img, idx) => (
                <div key={idx} className="space-y-1.5">
                  ...
                  {img.caption && (
                    <p
                      className="text-[10px] text-center opacity-80"
      ...
      const response = await leadService.submitLead(
        section.cardId || sectionId,
        {
          ...
          message: Object.entries(formData)
            .filter(
              ([key]) =>
                !["name", "email", "phone", "consentGiven"].includes(key),
            )
            .map(([key, val]) => `${key}: ${val}`)
            .join("\n"),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
        {fields.map((field) => (
          <div key={field.fieldId} className="flex flex-col space-y-1">
            <label
              className="text-[11px] font-bold uppercase opacity-60 tracking-wider mb-1"
              style={{ color: colors.text }}
            >
              {field.label}{" "}
            ...
            {field.type === "textarea" ? (
              <textarea
                className="w-full px-4 py-3 bg-black/2 dark:bg-white/2 border border-black/6 dark:border-white/6 rounded-[18px] text-sm focus:outline-none focus:ring-2 focus:bg-transparent transition-all"
                rows={3}
                style={{
                  color: colors.primary || "#000000",
        ...
        <div className="flex items-start space-x-2 pt-2">
          ...
          />
          <label
            htmlFor={`consent-${sectionId}`}
            className="text-[9px] opacity-80 leading-snug"
          >
            I consent to sharing my submitted details for follow-up
            communications.
          </label>
        </div>
        {errors.consentGiven && (
