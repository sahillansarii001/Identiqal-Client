...
import { X, Search, User, Link, MessageSquare, FormInput, Image as ImageIcon, MapPin, Video, Calendar, Share2 } from 'lucide-react';

const LIBRARY_COMPONENTS = [
  {
    category: 'Essentials',
    items: [
      { type: 'about', label: 'Profile Header', description: 'Your photo, name & title.', icon: User, color: 'bg-blue-500' },
      { type: 'links', label: 'Social Links', description: 'Connect all your profiles.', icon: Link, color: 'bg-purple-500' },
    ]
  },
  {
    category: 'Content',
    items: [
      { type: 'gallery', label: 'Image Gallery', description: 'Showcase your portfolio.', icon: ImageIcon, color: 'bg-emerald-500' },
      { type: 'video', label: 'Video Embed', description: 'YouTube or Vimeo videos.', icon: Video, color: 'bg-red-500' },
      { type: 'testimonials', label: 'Testimonials', description: 'What others say about you.', icon: MessageSquare, color: 'bg-amber-500' },
    ]
  },
  {
    category: 'Business',
    items: [
      { type: 'form', label: 'Contact Form', description: 'Capture visitor details.', icon: FormInput, color: 'bg-indigo-500' },
      { type: 'calendar', label: 'Book Meeting', description: 'Calendly integration.', icon: Calendar, color: 'bg-cyan-500' },
      { type: 'map', label: 'Location Map', description: 'Show your business address.', icon: MapPin, color: 'bg-orange-500' },
    ]
  }
];

export const BlockPickerModal = () => {
  const { isBlockPickerOpen, setBlockPickerOpen, addSection } = useCardBuilderStore();
  ...
  const activeGroup = searchQuery ? filteredGroups : filteredGroups.filter(g => g.category === activeTab);
        ...
        >
          ...
          <div className="flex items-center justify-between px-8 py-6 border-b border-gray-100">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Enhance your digital card with new features.</p>
            </div>
            <button 
              onClick={() => setBlockPickerOpen(false)}
                      ...
                      {group.items.map(item => (
                        ...
                        >
                          <div className="flex items-start gap-4">
                            <div className={`p-3 rounded-xl text-white ${item.color} shadow-sm group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
                              <item.icon size={24} />
                            </div>
                            <div className="flex-1 pr-16">
                              <h4 className="font-semibold text-gray-900 group-hover:text-primary transition-colors">{item.label}</h4>
